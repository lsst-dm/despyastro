...
def zipper_interp_cols(image, mask, interp_mask, **kwargs):
   ...
   :Optional parameters (passed as **kwargs)
    ...
    msg = msg + "with xblock=%s, yblock=%s, maxcols=%s, mincols=%s, add_noise=%s, ydilate=%s" % (
        xblock, yblock, max_cols, min_cols, add_noise, ydilate)
    if logger:
        logger.info(msg)
    else:
        print "#", msg
    ...
    for run in range(len(xstart)):
        ...
        if n_good > 0:
            ...
            msg = "WARNING: All sampling values equal to zero on slices [%s:%s,%s:%s] and [%s:%s,%s:%s]" % (
                y1a, y1b, x1, x2, y2a, y2b, x1, x2)
            if logger:
                logger.info(msg)
            else:
                print "#", msg
