...
def get_header(file):
    ...
    Returns a string containing all the lines at the top of a file which
    start by '#'.
    """
    buffer = ''
...
def rcols(file, cols=None, nrows='all'):
    """Make a table out of data.

    Returns data in the columns defined by the tuple (or single integer)
    cols as a tuple of float arrays (or a single float array).
    """
    if cols == None:
        nvar = 0
...
# Read/write 2D arrays (added from useful.py).
...
def put_2Darray(file, array, header='', format='', append='no'):
    ...
    Writes a 2D array to a file, where the first index changes along the
    lines and the second along the columns.

    Usage:
        put_2Darray(file,a,header,format)

    where header is any string and format is a string of the type: '%f %f %i'.
    """
    lista = []
